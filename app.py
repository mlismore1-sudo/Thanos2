"""Thanos Streamlit dashboard with database and worker health checks."""
...
def health_panel() -> None:
    ...
    worker_status, worker_row = worker_health()
    ...
    with col3:
        if worker_row:
            st.metric("Events committed", worker_row.get("events_committed", 0))
        else:
            st.metric("Events committed", 0)
